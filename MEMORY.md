...
### 2026-06-17 — Claude (Cowork, Opus) — Strategieanalyse: Hauptseite vs. Landingpages-Split

- **Anlass:** Giampiero hat strategisch hinterfragt, ob eine ruhige Haupt-/Trust-Seite (`/`) von aggressiven Funnel-/Landingpages getrennt werden sollte. Vollständige Analyse in `conversion-research/site-split-strategy-2026-06-17.md`.
- **Kernantwort:** Ja, Trennung ist sinnvoll — und wird unabhängig bestätigt durch `fabio-folder/GEO-AI/` (Agent-11-Workstream): Das dort vorbereitete Service-Schema (`ProAqua_Punkt04_Service_Schemas.md`) verlangt explizit eigene Unterseiten pro Service (AC Duct, Villa Deep Cleaning, Office Cleaning, Disinfection, Sofa/Mattress/Carpet) und enthält bereits eine Platzhalter→Ziel-URL-Tabelle. Passendes FAQ-Content (20 Q&A) liegt ebenfalls schon vor, explizit für Service-Unterseiten vorgesehen.
- **URL-Schema-Konflikt entdeckt:** GEO-AI-Schema nennt `/ac-duct-cleaning` etc. (ohne `/services/`-Präfix, ohne `-dubai`-Suffix, 5 Services). Empfehlung im Strategiedokument: `/services/<name>-dubai/` favorisieren, aber Giampiero muss final entscheiden, bevor die erste Service-Seite gebaut wird.
- **Wichtige Korrektur zu `conversion-strategy.md`:** Dieses Dokument ist veraltet — aktueller Code zeigt „10,000+" (nicht „6,000+") und „DED No. 748159" (nicht Platzhalter). `conversion-strategy.md` sollte bei nächster Gelegenheit aktualisiert werden.
- **Akuter Zusatzfund (unabhängig von der IA-Frage):** `sitemap.xml` listet 8 Location-Landingpages (`ac-cleaning-dubai-marina`, `ac-cleaning-jbr`, `ac-cleaning-downtown-dubai`, `ac-cleaning-business-bay`, `villa-cleaning-jumeirah`, `ac-cleaning-difc`, `deep-cleaning-mirdif`, `villa-cleaning-springs`, datiert 12.06.2026), die **nicht im Repo existieren**. Falls diese Sitemap bereits bei Google Search Console eingereicht ist, crawlt Google aktuell 8x 404. Sollte vorrangig behoben werden (Seiten bauen oder aus Sitemap entfernen).
- **Empfohlene Bau-Reihenfolge:** 1) Quiz extrahieren → `/lp/ac-risk-check/`, 2) Hauptseite entschärfen (Hero-Copy beruhigen, Warning-Signs/Hidden-Danger/aggressiven Compare-Block verschieben), 3) AC-Duct-Service-Seite (meiste fertige Inhalte), 4) Mattress-/Deep-Clean-Service-Seiten, 5) die 8 angekündigten Location-Seiten nachbauen, 6) `/ar/`-Parität erst danach.
- **Noch nicht umgesetzt** — dies war eine reine Strategie-/Bewertungsanfrage, keine Implementierung. Nächster Schritt liegt bei Giampiero: URL-Schema-Entscheidung treffen, dann Phase 1 freigeben.
